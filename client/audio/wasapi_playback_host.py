...
class WasapiPlaybackHostProcess:
    ...
    def configure(self, *, device_index: int | None, device_name: str, device_endpoint_id: str) -> None:
    ...
    async def start(self) -> None:
        ...
        if executable is None:
            raise RuntimeError("wasapi_playback_host_unavailable")
        ...
        try:
            ...
            await self.stop()
            raise RuntimeError(self.last_error or "wasapi_playback_host_start_failed")
        self.running = True
    ...
    async def enqueue(self, pcm_bytes: bytes) -> None:
        if not pcm_bytes or self._process is None or self._process.stdin is None:
            return
        payload = json.dumps(
            {"command": "play", "pcm_base64": base64.b64encode(pcm_bytes).decode("ascii")},
            ensure_ascii=False,
        ).encode("utf-8")
    ...
    async def _read_stdout(self) -> None:
        ...
        while True:
            ...
            try:
                ...
                self.last_error = f"invalid playback sidecar json: {raw_line}"
                continue
            await self._handle_payload(payload)
        if self._ready_future is not None and not self._ready_future.done():
            self._ready_future.set_exception(RuntimeError(self.last_error or "wasapi_playback_host_exited"))
    ...
    async def _read_stdout_payload_for_test(self, payload: dict[str, object]) -> None:
        await self._handle_payload(payload)
